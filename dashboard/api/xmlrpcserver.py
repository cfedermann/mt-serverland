...
class XmlRpcAPIServer(object):
    ...
    def start_server(self):
        ...
        print 'Dashboard API lives at {}'.format(self.api_url)
    ...
    def list_workers(self, token, shortname = None):
        ...
        if shortname is not None:
            url = self.api_url + 'workers/{}/?token={}'.format(shortname, token)
        else:
            url = self.api_url + 'workers/?token={}'.format(token)
    ...
    def list_requests(self, token, shortname_or_request_id = None):
        ...
        if shortname_or_request_id is not None:
            url = self.api_url + 'requests/{}/?token={}'.format(
                shortname_or_request_id, token)
        else:
            url = self.api_url + 'requests/?token={}'.format(token)
        response = self.http.request(url, method='GET')
        if response[0].status == 200:
            return json.loads(response[1])
        else:
    ...
    def list_results(self, token, shortname_or_request_id = None):
        ...
        '''
        if shortname_or_request_id is not None:
            url = self.api_url + 'results/{}/?token={}'.format(
                shortname_or_request_id, token)
        else:
            url = self.api_url + 'results/?token={}'.format(token)
        response = self.http.request(url, method='GET')
        if response[0].status == 200:
            return json.loads(response[1])
        else:
